...
def make_mock_page(url="https://www.facebook.com/home"):
    page = AsyncMock()
    page.url = url
...
class TestIsCheckpointActive:
    ...
    async def test_checkpoint_url_detected(self):
        page = make_mock_page(url="https://www.facebook.com/checkpoint/123456")
        walker = make_mock_dom_walker()
    ...
    async def test_challenge_url_detected(self):
        page = make_mock_page(url="https://www.facebook.com/challenge/789")
        walker = make_mock_dom_walker()
...
class TestHandleCheckpointIfNeeded:
    async def test_takes_screenshot_when_detected(self):
        ...
        assert "test@fb.com" in screenshot_kwargs["path"]
