...
class FBActions:
    ...
    # ── New actions for Workflow nodes ──

    async def publish_on_fanpage(self, fanpage_url: str, text: str,
                                  background_style: str = None) -> bool:
        """Publish a text post on a Facebook fanpage."""
        ...
        try:
            # Find the fanpage composer (textbox on the page, not in a group)
            from .dom_walker import _eval_js
            await HumanImitation.natural_scroll(self.tab, scrolls=1)
            await _eval_js(self.tab, "window.scrollTo(0, 0)")
            await HumanImitation.human_delay(0.5, 1.0)

            # Look for composer textbox or "What's on your mind" trigger
            composer = await self.dom.find(
                "div[role='button'][tabindex='0']", timeout=5.0
            )
            if not composer:
                composer = await self.dom.find(
            ...
            if not composer:
                raise TimeoutError("Nie znaleziono composera na fanpage")

            await mouse_engine.click_element(self.tab, composer)
            await HumanImitation.human_delay(1, 3)

            # Find text editor in dialog
            editor = await self.dom.find(
                "div[role='dialog'] div[role='textbox'][contenteditable='true']",
                timeout=5.0,
            )
            if not editor:
                # Try without dialog (some fanpage layouts don't use dialog)
                editor = await self.dom.find(
            ...
            if background_style:
                text = text[:100]
                await self._select_post_background(background_style)
                editor = await self.dom.find(
                    "div[role='textbox'][contenteditable='true']", timeout=3.0
                )
            ...
            logger.info("Post na fanpage opublikowany")
            return True
